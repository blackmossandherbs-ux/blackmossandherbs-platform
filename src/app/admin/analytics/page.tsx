            ...
            <div className="container relative z-10">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
                    <div>
                        ...
                        <p className="text-earth-400 text-lg">Financial Intelligence & Entity Performance Metrics</p>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
                    {stats.map((stat, idx) => (
                        <div key={idx} className="card p-8 border border-earth-800 bg-earth-900/40 backdrop-blur-xl group hover:border-primary-500/50 transition-all">
                            <div className="flex justify-between items-start mb-6">
                                ...
                                <span className={`text-xs font-black ${stat.sub.startsWith('+') ? 'text-secondary-400' : 'text-red-400'}`}>
                                    {stat.sub}
                                </span>
                            </div>
                            <h3 className="text-earth-500 text-[10px] font-black uppercase tracking-[0.2em] mb-2">{stat.label}</h3>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 card border border-earth-800 bg-earth-900/60 backdrop-blur-xl p-10 rounded-[2.5rem]">
                        ...
                        <div className="h-80 w-full bg-earth-950/50 rounded-3xl border border-earth-800 flex items-center justify-center relative overflow-hidden">
                            <div className="absolute inset-0 opacity-20 pointer-events-none">
                                ...
                                <svg className="w-full h-full" preserveAspectRatio="none">
                                    <path d="M0 200 Q 100 150 200 180 T 400 100 T 600 120 T 800 50 L 800 400 L 0 400 Z" fill="url(#gradient)" />
                                    <defs>
                            ...
                            <p className="text-earth-600 font-black text-xs uppercase tracking-[0.4em] relative z-10">Chart Synchronization Active</p>
                        </div>
                        ...
                        <div className="mt-8 grid grid-cols-3 gap-6">
                            ...
                            <div className="text-center">
                                <div className="text-[10px] text-earth-500 font-black uppercase mb-1">Lifetime Value</div>
                                <div className="text-xl font-bold text-white">£842.00</div>
                            </div>
                            <div className="text-center">
                                <div className="text-[10px] text-earth-500 font-black uppercase mb-1">Churn Percent</div>
                    ...
                    <div className="lg:col-span-1 space-y-6">
                        <div className="card border border-earth-800 bg-earth-900/40 backdrop-blur-xl p-8 rounded-[2rem]">
                            ...
                            <div className="space-y-6">
                                {topEntities.map((entity, idx) => (
                                    <div key={idx} className="flex items-center justify-between p-4 bg-earth-950/50 rounded-2xl border border-earth-800 hover:border-secondary-500/30 transition-all cursor-pointer group">
                                        <div>
                                            <div className="font-bold text-sm text-white group-hover:text-secondary-400 transition-colors">{entity.name}</div>
                                            <div className="text-[10px] font-black text-earth-500 uppercase tracking-widest mt-1">{entity.status}</div>
                                        </div>
                                        <div className="text-right">
                                            <div className="font-bold text-white">{entity.traffic}</div>
                                            <div className={`text-[10px] font-black ${entity.growth.startsWith('+') ? 'text-secondary-400' : 'text-red-400'}`}>
                                                {entity.growth}
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <Button className="w-full mt-8 py-4 rounded-xl text-[10px] font-black uppercase tracking-[0.2em]" variant="outline">
                        ...
                        <div className="card border border-earth-800 bg-primary-900/5 backdrop-blur-xl p-8 rounded-[2rem] relative overflow-hidden">
                            ...
                            <p className="text-earth-400 text-sm mb-6 leading-relaxed">Based on current velocity, MRR is projected to reach <span className="text-primary-400 font-bold">£50K</span> within the next 45 diurnal cycles.</p>

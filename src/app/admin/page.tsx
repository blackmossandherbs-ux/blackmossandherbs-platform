      ...
      <div className="container relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          {[
            ...
            <div
              key={index}
              className="card p-8 border border-earth-800 bg-earth-900/40 backdrop-blur-xl relative group hover:border-secondary-500/50 transition-all"
            >
              <div className="flex items-center justify-between mb-6">
                <div
                  className={`w-14 h-14 bg-${stat.color}-900/20 rounded-2xl flex items-center justify-center border border-${stat.color}-700/30 group-hover:scale-110 transition-transform`}
                >
              ...
              <div className="text-sm font-medium text-earth-400 uppercase tracking-widest">
                {stat.label}
              </div>

              {/* Subtle Glow */}
              <div
                className={`absolute -bottom-2 -left-2 w-24 h-24 bg-${stat.color}-500/5 blur-3xl rounded-full`}
              />
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="card border border-earth-800 bg-earth-900/60 backdrop-blur-xl p-8">
            <div className="flex items-center justify-between mb-8">
              ...
              <button className="text-secondary-400 text-sm font-bold hover:underline">
                View All
              </button>
            </div>
            <div className="space-y-4">
              {[
                ...
                { id: 'ORD-003', customer: 'Bob Johnson', amount: 124.5, status: 'Delivered' },
                { id: 'ORD-004', customer: 'Alice Brown', amount: 34.99, status: 'Pending' },
              ].map((order) => (
                <div
                  key={order.id}
                  className="flex items-center justify-between p-5 bg-earth-950/50 border border-earth-800/50 rounded-2xl hover:border-earth-700 transition-colors"
                >
          ...
          <div className="card border border-earth-800 bg-earth-900/60 backdrop-blur-xl p-8">
            ...
            <div className="space-y-4">
              {[
                ...
                <div
                  key={index}
                  className="flex items-center justify-between p-5 bg-earth-950/50 border border-earth-800/50 rounded-2xl"
                >
                  <div className="flex items-center gap-4">
                  ...
                  <div className="font-bold text-primary-400">£{product.revenue.toFixed(2)}</div>
                </div>
              ))}
            </div>
          </div>

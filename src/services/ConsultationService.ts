...
export class ConsultationService {
  ...
  static getPrice(type: ConsultationType): number {
    const pricing = {
      [ConsultationType.PRIVATE_1ON1]: 149.0,
      [ConsultationType.ASYNC_PROTOCOL]: 89.0,
      [ConsultationType.EMERGENCY_AUDIT]: 49.0,
    };
    return pricing[type];
  }
  ...
  static async getUserConsultations(userId: string) {
    try {
      ...
      console.error(
        `[ConsultationService.getUserConsultations] FAILURE for user ${userId}:`,
        error
      );
      return [];
    }
  ...
  static async updateStatus(consultationId: string, status: ConsultationStatus) {
    try {
      ...
      console.error(
        `[ConsultationService.updateStatus] FAILURE for consultation ${consultationId}:`,
        error
      );

...
echo "  http://$SERVER_IP:3005"
echo ""
echo "⚠️  IMPORTANT: This server hosts multiple sites!"
echo "    Black Moss & Herbs runs on port 3005"
echo "    Other sites may be using port 3000"
echo ""
echo "Next steps:"
echo "  1. Configure your domain DNS to point to $SERVER_IP"
echo "  2. Copy nginx config: cp config/blackmoss.nginx.conf /etc/nginx/sites-available/"
echo "  3. Enable site: ln -s /etc/nginx/sites-available/blackmoss.nginx.conf /etc/nginx/sites-enabled/"
echo "  4. Run SSL setup: certbot --nginx -d blackmossandherbs.com -d www.blackmossandherbs.com"
echo ""
